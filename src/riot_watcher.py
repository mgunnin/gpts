...
parser.add_argument(
    "-v", "--verbose", help="Verbose mode", required=False, default=False
)
parser.add_argument("-u", "--username", help="Player username", required=True)
...
for player in players_to_analyze:
    ...
    for x in matches:
        file_names = [
            "data/{}_{}.txt".format(player, x),
            "data/{}_{}_timeline.txt".format(player, x),
        ]
        if os.path.isfile(file_names[0]):
            # Skip iteration as we have already processed this call.
        ...
        match_data = {"id": x, "data": match_info}
        timeline_data = {"id": x, "data": match_timeline}
